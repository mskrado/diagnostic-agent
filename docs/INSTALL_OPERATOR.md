...
1. Read and follow the project docs: docs/INSTALL.md, docs/WORKSPACE.md,
   docs/INTEGRATING.md, docs/INSTALL_OPERATOR.md (and eval/README.md only if
   running blind eval).
2. Discover the running system with evidence (docker ps / compose, published
   ports, Prom/Loki/AM URLs, existing metric names and Loki service= labels) —
...
- Prefer evidence (command output, file contents, install-report.json, APPLY.md)
  over guesses. Verify PromQL templates against live series before trusting a
  preset (e.g. confirm histogram buckets exist if using histogram_quantile).
...
- If another agent container already uses port 8001 / name diagnostic-agent,
  pick a distinct host port and container_name (compose already pins a unique
  project name from the output directory).

## What you may automate
- Running install / dry-run / validate / lint
- Reading APPLY.md + install-report.json
- Inspecting docker/compose and suggesting service_map + preset
- Relying on install seeding: spring-micrometer copies
  examples/spring-modular-monolith/; generic gets stubs + starter map;
  blind_eval.yaml and the full runbook corpus are included
- Proposing .env URL and Mailpit SMTP defaults when discovery supports them
- Pinning AGENT_PROFILE_DIR=/workspace if editing compose by hand
...
1. **Discover** — docker/compose, ports, existing Prom/Loki/AM; sample metric
   names and Loki `service=` values.
...
4. **Patch workspace** — tune `service_map` / profiles for this stack; keep
   fail-closed redaction. Prefer evidence-backed PromQL overrides when the
   preset templates would return empty (missing buckets, client-side metric
   labels, etc.).
...
6. **Start + health** — `docker compose --env-file .env up -d` then `curl /health`.
   ...
   runbook corpus.
7. **Optional** — one test `POST /alert`, or blind eval from the install tree:
