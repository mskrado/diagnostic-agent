...
def generate(
    *,
    ...
    plan.append(
        (workspace / "metrics_profile.yaml", _profile_stub("metrics_profile", params.preset))
    )
    plan.append(
        (workspace / "logs_profile.yaml", _profile_stub("logs_profile", params.preset))
    )
    plan.append(
        (workspace / "prompt_profile.yaml", _profile_stub("prompt_profile", params.preset))
    )
    plan.append(
        (workspace / "redaction.yaml", _profile_stub("redaction", params.preset))
    )
    ...
    # generic hosts get thin stubs + a starter service_map.
    ...
    for src in sorted(runbooks_src.glob("runbook-*.md")):
        plan.append((workspace / "runbooks" / src.name, src.read_text(encoding="utf-8")))
    readme = runbooks_src / "README.md"
    if readme.is_file():
        plan.append((workspace / "runbooks" / "README.md", readme.read_text(encoding="utf-8")))
    ...
    if blind_src.is_file():
        plan.append((workspace / "blind_eval.yaml", blind_src.read_text(encoding="utf-8")))
    else:
...
#   runbooks / scenarios. The agent loads this when AGENT_WORKSPACE points here.
#
# HOW THE AGENT USES IT
#   `extends` selects built-in PromQL/log naming (generic-prometheus or
#   spring-micrometer). `profile: .` means the YAML files beside this manifest
#   are the integration profile (flat layout).
#
# CONFIGURE
...
#   3. Optionally set agent_version and blind_eval: ./blind_eval.yaml.
#   4. Do not point profile/runbooks at missing paths you declare.
...
"""
    ...
    return (
        ...
        + """\
    return (
        "# Generated by diag install - self-sufficient host workspace.\n"
        "# Run validate / lint / eval against this directory only (no host monorepo).\n"
        "schema: 1\n"
    ...
    if preset == "spring-micrometer":
        ...
        for name in (
            ...
            if src.is_file():
                ...
                files.append((name, f"extends: {preset}\n"))
        return files

    return [
        ("metrics_profile.yaml", f"extends: {preset}\n"),
        ("logs_profile.yaml", f"extends: {preset}\n"),
        ("prompt_profile.yaml", f"extends: {preset}\n"),
        ("redaction.yaml", f"extends: {preset}\n"),
...
def _apply_md(params: InstallParams, report: DiscoveryReport) -> str:
    ...
    lines += [
        ...
        "",
        "## 6. Blind eval (self-contained — no host monorepo)",
        "",
