...
from .models import DiscoveryReport, InstallParams, client_image_ref
...
def generate(
    *,
    ...
    layout:
      bundle — ``agent/workspace`` under output (``diag install`` default)
      client — ``workspace`` at output root, ``agent`` holds compose only (``diag init``)
    """
    ...
    workspace = output / "workspace" if client_layout else agent_dir / "workspace"
    plan.append((agent_dir / "Dockerfile", _dockerfile(params, client_layout)))
    plan.append(
        (agent_dir / "docker-compose.yml", _compose(params, output, client_layout))
    )
    env_content = _env_file(params)
    plan.append((agent_dir / ".env", env_content))
    if client_layout:
        plan.append((agent_dir / ".env.example", _env_example(params)))
...
# templates
# ---------------------------------------------------------------------------
def _dockerfile(params: InstallParams, client_layout: bool = False) -> str:
    if params.build_from_source:
        context = "../.." if client_layout else "."
        return f"""\
# Build diagnostic-agent from the repository source (client fork / air-gapped).
# Generated by `diag init` / `diag install`.
#
...
"""
    return """\
# Pull the prebuilt image (preferred when online):
#   docker pull ghcr.io/mskrado/diagnostic-agent:latest
#
# This Dockerfile is a thin wrapper when not building from source.
...
def _compose(params: InstallParams, output: Path, client_layout: bool = False) -> str:
    network_block = ""
    service_network = ""
...
"""
    ...
    image_ref = client_image_ref(params)
    workspace_mount = "../workspace:/workspace:ro" if client_layout else "./workspace:/workspace:ro"
    ...
    return f"""\
# Generated by `diag install` / `diag init`.
#
# HOW TO USE
...
#
# WHAT THIS FILE DOES
#   Runs the diagnostic-agent, injects agent/.env, and bind-mounts
#   workspace -> /workspace:ro. Audit JSONL and Chroma RAG persist in named volumes.
#
# CONFIGURE
...
services:
  diagnostic-agent:
{service_image}{build_block}    container_name: {params.agent_container_name}
    ports:
...
def _env_example(params: InstallParams) -> str:
    """Sanitized .env for version control (no secrets)."""
    secret_keys = {
        "AGENT_GRAFANA_TOKEN",
        "AGENT_SMTP_PASSWORD",
        "OPENAI_API_KEY",
    ...
    lines: list[str] = []
    for line in _env_file(params).splitlines():
        ...
        if key in secret_keys:
            ...
            lines.append(line)
    lines.insert(1, "# Copy to .env and fill secrets: cp .env.example .env")
    return "\n".join(lines) + "\n"
